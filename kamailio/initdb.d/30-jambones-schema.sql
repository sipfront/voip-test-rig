-- jambonz (jambones) database — schema + dedicated app user.
--
-- Loaded by the shared mysql container's initdb on first boot, AFTER the kamailio
-- schema/seed (00-/10-). The jambonz components connect with
-- JAMBONES_MYSQL_USER/PASSWORD/DATABASE = jambones (see docker-compose.yml).
--
-- The schema below is jambonz-api-server db/jambones-sql.sql (vendored verbatim;
-- update it together with the jambonz image tags). mysql_native_password keeps the
-- jambonz mysql2 client happy without public-key retrieval over the plain docker net.

CREATE DATABASE IF NOT EXISTS jambones CHARACTER SET utf8mb4;
CREATE USER IF NOT EXISTS 'jambones'@'%' IDENTIFIED WITH mysql_native_password BY 'jambones';
GRANT ALL PRIVILEGES ON jambones.* TO 'jambones'@'%';
FLUSH PRIVILEGES;

USE jambones;

-- ===========================================================================
-- vendored: github.com/jambonz/jambonz-api-server db/jambones-sql.sql
-- ===========================================================================
/* SQLEditor (MySQL (2))*/
